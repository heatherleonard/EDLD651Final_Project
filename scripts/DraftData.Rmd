...
synthdata <- synthdata %>%
# jftn - another way to do this, but this will not create new variables, if that is important you can do that too
  # mutate(across(c(io1_bl:io4_bl), ~recode(.x,
  #                                         "i don't know" = 0, "never" = 1, "often" = 2, "rarely" = 3, "sometimes" = 4)))
  mutate(io1_RC = recode(io1_bl, "i don't know" = 0, "never" = 1, "often" = 2, "rarely" = 3, "sometimes" = 4),
         io2_RC = recode(io2_bl, "i don't know" = 0, "never" = 1, "often" = 2, "rarely" = 3, "sometimes" = 4),
         io3_RC = recode(io3_bl, "i don't know" = 0, "never" = 1, "often" = 2, "rarely" = 3, "sometimes" = 4),
         io4_RC = recode(io4_bl, "i don't know" = 0, "never" = 1, "often" = 2, "rarely" = 3, "sometimes" = 4),
         io_comp = io1_RC + io2_RC + io3_RC + io4_RC,
         # age_RC = parse_number(dm_childage_bl), #You could try this instead of the line below, but nice use of str_sub()!
         age_RC = as.numeric(str_sub(synthdata$dm_childage_bl, 1, 2)),
         lunch_RC = recode(dm_lunch_bl, "no" = 0, "yes" = 1,"i don't know" = 2))
...
#Mean and SD
iomean <- round(mean(synthdata$io_comp, na.rm = TRUE), 2)

iosd <- round(sd(synthdata$io_comp, na.rm = TRUE), 2)
```
...
## Methods
Data were collected from `r nrow(synthdata)` middle school students in the Pacific Northwest. Parents were notified by mail and given the option to opt their student out of the survey. Student assent was collected by research assistants in the classroom prior to administering the survey. Students were compensated for their participation in the study. All procedures were approved by the institutional and school district review boards.

Demographic data collected included age, sex, race/ethnicity, and free/reduced lunch status (used as a stand-in for socioeconomic status). Students were ages `r min(synthdata$age_RC, na.rm = TRUE)`-`r max(synthdata$age_RC, na.rm = TRUE)`, `r round(sum(synthdata$dm_sex_bl == "male")/nrow(synthdata)*100)`% male and 47% female, and `r round(sum(synthdata$dm_lunch_bl == "yes")/nrow(synthdata)*100)`% were receiving free/reduced lunch. Ethnicity included 26% identifying as Hispanic or Latino, and race included 33% reporting more than one race, 32% white, 16% unknown, 5% American Indian/native Alaskan, and 3% black/African American.
...
We used the following `R` @rsoftware packages on this assignment: `tidyverse` @wickham2019welcome, `rio` @riopackage, `gtsummary` @gtsummary, and `here` @herepackage.


```{r pivotlonger-to-tidy-iodata, eval=FALSE}
synthdata %>%
...
```{r grouping-and-summarizing-using-pivot-wider, eval=FALSE}
synthdata %>% 
...
cor1 <- cor.test(synthdata$sl_bedmedia_bl, synthdata$io_comp, method = "pearson", use = "complete.obs")

#positive relationship that is very very small

```

We ran a Pearson's correlation analysis to examine the relationship between bedtime media use and internet overuse. There was significant,  positive association *r*(`r unname(cor1$parameter)`) = `r round(unname(cor1$estimate), 2)`, *p* < .01. As adolescent bedtime media use increases, so do adolescent reports of overusing the internet.  
...
#school night sleep
cor2 <- cor.test(synthdata$io_comp, synthdata$sl_sleepschool_bl, method = "pearson", use = "complete.obs")
#not really a correlation between internet overuse and school night sleep on school nights

#daytime sleepiness
cor3 <- cor.test(synthdata$io_comp, synthdata$sl_sleepy_bl, method = "pearson", use = "complete.obs")
#positive correlation that is very small
```

To examine the relationship between internet overuse and sleep outcomes, we ran two Pearson's correlation analyses. First, we tested the association between internet overuse and average sleep on school nights. The results indicated that these two variables are not significantly associated *r*(`r unname(cor2$parameter)`) = `r round(unname(cor2$estimate), 2)`, _p_  = .46. However, internet overuse was positively correlated with reported daytime sleepiness *r*(335) = .18, _p_  < .001, indicating that as reports of internet overuse increase, reports of daytime sleepiness also increase.   
...
synthdata <- synthdata %>%
  filter(
    synthdata$age_RC != "i don't know" & 
      synthdata$dm_lunch_bl != "i don't know")
...
These results suggest that bedtime media use may lead to internet overuse among adolescents. Additionally, internet overuse may increase daytime sleepiness, but does not impact total hours of sleep. These findings indicate that media overuse in general may be harmful to quality of life for adolescents and that media devices are not as innocuous as they appear to be.
